...
```
Focus the most recently used application (excluding Safari and Preview) using left_command + r :
...
"Focus the most recently used application by left_command + r"
,
...
You can find the bundle identifier and file path in EventViewer > Frontmost Application .
- The frontmost_application_history_index should be set as an integer >= 1.
- When frontmost_application_history_index is specified, the selected application will be the one that was recently focused. Applications opened through methods other than open_application , such as via Launchpad, are also included.
...
- When you also specify frontmost_application_history_exclusion_bundle_identifiers or frontmost_application_history_exclusion_file_paths , you can exclude particular applications. The values should be regular expressions that match the bundle identifier or file path. These filters are available since Karabiner-Elements v15.7.3.