# Models
Zed's plans offer hosted versions of major LLMs with higher rate limits than direct API access. Model availability is updated regularly. To use your own API keys instead, see LLM Providers . For general setup, see Configuration .
> Note:
> Claude Opus models and GPT-5.4 pro are not available on the
> Student plan
> .
