...
> Note: API keys are
> not
> stored as plain text in your settings file, but rather in your OS's secure credential storage.
## Supported Providers
...
> Amazon Bedrock's Tool Use documentation
> .
...
2. Open your settings file ( zed: open settings file ) and include the bedrock key under language_models with the following settings: { "language_models": { "bedrock": { "authentication_method": "named_profile", "region": "your-aws-region", "profile": "your-profile-name" } } }
...
#### Custom Models
The Zed agent comes pre-configured to use DeepSeek V4 Flash and DeepSeek V4 Pro. If you wish to use alternate models or customize the API endpoint, you can do so by adding the following to your Zed settings file ( how to edit ):
```
{
  "language_models": {
    "deepseek": {
...
> Note
> : If you don't see specific models in the dropdown, you may need to enable them in your
...
2. In the app press cmd/ctrl-shift-m and download at least one model (e.g., qwen2.5-coder-7b). Alternatively, you can get models via the LM Studio CLI: lms get qwen2.5-coder-7b
3. Make sure the LM Studio API server is running by executing: lms server start
...
1. Download one of the available models , for example, for mistral : ollama pull mistral
2. Make sure that the Ollama server is running. You can start it either via running Ollama.app (macOS) or launching: ollama serve
3. In the Agent Panel, select one of the Ollama models using the model dropdown.
#### Ollama Autodiscovery
Zed will automatically discover models that Ollama has pulled. You can turn this off by setting the auto_discover field in the Ollama settings. If you do this, you should manually specify which models are available.
```
{
  "language_models": {
    "ollama": {
...
> Note
> : Token counts displayed in the Agent Panel are only estimates and will differ from the model's native tokenizer.
...
> : If
> context_window
> is set, it overrides any per-model
> max_tokens
> values.
...
The supports_tools option controls whether the model will use additional tools. If the model is tagged with tools in the Ollama catalog, this option should be supplied, and the built-in profiles Ask and Write can be used. If the model is not tagged with tools in the Ollama catalog, this option can still be supplied with the value true ; however, be aware that only the Minimal built-in profile will work.
The supports_thinking option controls whether the model will perform an explicit "thinking" (reasoning) pass before producing its final answer. If the model is tagged with thinking in the Ollama catalog, set this option and you can use it in Zed.
The supports_images option enables the model's vision capabilities, allowing it to process images included in the conversation context. If the model is tagged with vision in the Ollama catalog, set this option and you can use it in Zed.
...
One such service is Ollama Turbo . To configure Zed to use Ollama Turbo:
1. Sign in to your Ollama account and subscribe to Ollama Turbo
2. Visit ollama.com/settings/keys and create an API key
...
#### Custom Models
The Zed agent comes pre-configured to use the latest version for common OpenAI models (GPT-5.2, GPT-5 mini, GPT-5.2 Codex, and others). To use alternate models, perhaps a preview release, or if you wish to control the request parameters, you can do so by adding the following to your Zed settings file ( how to edit ):
```
{
  "language_models": {
    "openai": {
...
To do it via the UI, go to the Agent Panel settings ( agent: open settings ) and look for the "Add Provider" button to the right of the "LLM Providers" section title. Then, fill up the input fields available in the modal.
To do it via your settings file ( how to edit ), add the following snippet under language_models :
```
{
  "language_models": {
    "openai_compatible": {
...
Note that LLM API keys aren't stored in your settings file. So, ensure you have it set in your environment variables ( <PROVIDER_NAME>_API_KEY=<your api key> ) so your settings can pick it up. In the example above, it would be TOGETHER_AI_API_KEY=<your api key> .
### OpenCode
OpenCode offers multiple ways to access AI models:
- OpenCode Zen : a pay-as-you-go subscription with access to a large number of tested and verified models
- OpenCode Zen Free : free access to a limited set of models, with data and feedback collected to improve the models
- OpenCode Go : a low-cost monthly subscription with access to a validated set of open coding models
...
```
#### Custom Models
The Zed agent comes pre-configured with OpenCode models. If you wish to use newer models or models with custom endpoints, you can do so by adding the following to your Zed settings file ( how to edit ):
```
{
  "language_models": {
    "opencode": {
...
#### Custom Models
You can add custom models to the OpenRouter provider by adding the following to your Zed settings file ( how to edit ):
```
{
  "language_models": {
    "open_router": {
...
### Vercel AI Gateway
Vercel AI Gateway provides access to many models through a single OpenAI-compatible endpoint.
...
```
### Vercel v0
Vercel v0 is a model for generating full-stack apps, with framework-aware completions for stacks like Next.js and Vercel. It supports text and image inputs and provides fast streaming responses.
The v0 models are OpenAI-compatible models , and Vercel appears as a dedicated provider in the panel's settings view.
To start using it with Zed, ensure you have first created a v0 API key . Once you have it, paste it directly into the Vercel provider section in the panel's settings view.
You should then find it as v0-1.5-md in the model dropdown in the Agent Panel.
### xAI
Zed includes a dedicated xAI provider. You can use your own API key to access Grok models.
...
> Note:
> The xAI API is OpenAI-compatible, and Zed also includes a dedicated xAI provider. We recommend using the dedicated
> x_ai
> provider configuration instead of the
> OpenAI API Compatible
> method.
#### Custom Models
The Zed agent comes pre-configured with common Grok models. If you wish to use alternate models or customize their parameters, you can do so by adding the following to your Zed settings file ( how to edit ):
```
{
  "language_models": {
    "x_ai": {
...
You can use a custom API endpoint for different providers, as long as it's compatible with the provider's API structure. To do so, add the following to your settings file ( how to edit ):
```
{
  "language_models": {
    "some-provider": {
