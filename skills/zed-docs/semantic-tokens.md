...
> Changing the
> semantic_tokens
> mode may require a language server restart to take effect. Use the
...
Rules are matched in order, and the first matching rule wins. User-defined rules take highest precedence, followed by extension-provided language rules, then Zed defaults.
### Rule Structure
...
For the complete specification, see the LSP Semantic Tokens documentation .
...
4. Check the LSP logs ( dev: open language server logs ) for errors
