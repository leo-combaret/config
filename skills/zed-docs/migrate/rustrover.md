...
```
You can also launch Zed from the terminal inside any existing Cargo project with: zed .
...
Run configurations don't transfer. RustRover stores run/debug configurations in .idea . These have no automatic migration path. You'll recreate them as Zed tasks in .zed/tasks.json and debug configurations in .zed/debug.json .
