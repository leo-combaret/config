...
The official documentation for the debug field is here . In short, "limited" strips type-level and variable-level debug info.
...
> on every
> cargo
> command, you can also change the section in the
> root
> Cargo.toml
> from
> [profile.release]
...
### Background
When you install Rust through rustup (the recommended setup for Zed development; see your platform guide here ), rustup also installs helper scripts for debugging Rust binaries.
...
The linked article notes that the minimum supported versions are GDB 7.7 and LLDB 310. In practice, newer versions are usually better.
> Note
> :
> rust-gdb
> is not installed by default on Windows because
> gdb
> support there is unstable. Use
> rust-lldb
> instead.
If you are new to these tools, see the gdb docs here and the lldb docs here .
### Usage with Zed
