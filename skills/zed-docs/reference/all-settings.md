...
```
1. Allow rewrap in selections only:
```
{
  "allow_rewrap": "in_selections"
}
```
1. Allow rewrap anywhere:
```
{
...
```
1. Show errors, warnings, and information:
```
{
  "diagnostics_max_severity": "info"
}
```
1. Show all including hints:
```
{
...
```
1. Open the excerpt clicked as a new buffer in the new tab:
```
{
...
```
1. Hide on typing and on key bindings that resolve to an action:
```
{
...
```
1. Do not show snippets in the completion list at all:
```
{
...
1. Whether or not to skip worktree and workspace trust checks:
```
{
  "session": {
    "trust_all_worktrees": false
...
- notifications : Notification-related settings. dismiss_timeout_ms : Timeout in milliseconds for automatically dismissing language server notifications. Set to 0 to disable auto-dismiss.
...
```
1. Use Copilot as the edit prediction provider:
```
{
  "edit_predictions": {
    "provider": "copilot"
...
```
1. Turn off edit predictions across all providers
```
{
  "edit_predictions": {
    "provider": "none"
...
1. To disable the formatter, use "none" . This setting disables the configured formatter, but any actions in code_actions_on_format will still be executed:
```
{
  "formatter": "none"
}
...
```
1. Set the padding between the end of the line and the inline blame hint, in ems:
```
{
  "git": {
    "inline_blame": {
...
```
1. Find references for the same symbol (default):
```
{
...
```
1. Scroll the minimum amount needed to make the target visible:
```
{
  "go_to_definition_scroll_strategy": "minimum"
}
```
1. Scroll so the target appears near the top of the viewport:
```
{
...
- base : What settings to start from before applying the profile's overrides. "user" (default): Apply on top of your current user settings. "default" : Apply on top of Zed's default settings, ignoring user customizations.
...
```
1. Show relative line numbers in the gutter, including wrapped lines in the counting:
```
{
  "relative_line_numbers": "wrapped"
}
```
1. Do not use relative line numbers:
```
{
...
```
1. Always start with the welcome launchpad:
```
{
...
```
1. The editor will scroll beyond the last line by the same amount of lines as vertical_scroll_margin :
```
{
  "scroll_beyond_last_line": "vertical_scroll_margin"
}
```
1. The editor will not scroll beyond the last line:
```
{
...
```
1. A box drawn around the following character
```
{
  "terminal": {
    "cursor_shape": "hollow"
...
```
1. Use a standard line height, 1.3. This option is useful for TUIs, particularly if they use box characters. (default)
```
{
  "terminal": {
...
```
1. Use the first project in this workspace's directory. Fallback to using this platform's home directory.
```
{
  "terminal": {
...
```
1. Always use this platform's home directory if it can be found.
```
{
  "terminal": {
...
```
1. Always close the window:
```
{
  "when_closing_with_no_tabs": "close_window"
}
```
1. Never close the window:
```
{
...
```
1. Mix directories and files together
```
{
  "project_panel": {
    "sort_mode": "mixed"
...
```
1. Show files first, then directories
```
{
  "project_panel": {
...
```
1. Uppercase names are grouped before lowercase names, with case-insensitive natural sort within each group. Dot-prefixed names (e.g., .gitignore ) sort before both groups. Accented uppercase letters like É are treated as uppercase.
```
{
  "project_panel": {
    "sort_order": "upper"
...
```
1. Lowercase names are grouped before uppercase names, with case-insensitive natural sort within each group. Dot-prefixed names sort before both groups.
```
{
  "project_panel": {
    "sort_order": "lower"
...
```
1. Pure Unicode codepoint comparison. No case folding and no natural number sorting. Uppercase ASCII letters ( A – Z ) sort before lowercase ( a – z ) as a natural consequence of their codepoint values. Accented characters like É (U+00C9) sort after all ASCII letters. Numbers are compared lexicographically ( file10 sorts before file2 ).
```
{
  "project_panel": {
    "sort_order": "unicode"
